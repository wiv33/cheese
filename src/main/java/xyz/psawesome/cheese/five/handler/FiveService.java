...
//        this.finalBusProcessor = Sinks.many().multicast().onBackpressureBuffer();
        ...
        resultOperations.searchLastResult(FiveType.POWER)
                .doOnNext(powerBusProcessor::tryEmitNext)
                .subscribe();
    }
    ...
    public Mono<FiveStepDto.NextResponse> nextResponse2(String userId, String subnetId, FiveType fiveType) {
        ...
        // 4. 정답이면 금액 초기화
        // 4. 오답이면 금액 증가
        return powerBusProcessor.asFlux().log("bus start ->>>>> ")
                .next().log(" bus end ->>> ")
                .doOnNext(res -> log.info("last result algorithm: {}, res: {}", res.getAlgorithm(), res))
                .zipWith(stepOperations.searchLastMono(userId, subnetId, fiveType)
                        .onErrorReturn(FiveStepDocument.forInitSave(userId, subnetId, "", fiveType)))
                .flatMap(lastZip -> {
                    var result = lastZip.getT1();
                    var step = lastZip.getT2();
                    if (isBlank(step.getAlgorithm()))
                        step.setAlgorithm(result.getAlgorithm());

                    return step.isAlreadyBat(result) ?
                            stepOperations.searchByIdMono(step.getFiveStepId())
                                    .onErrorResume(throwable -> stepOperations.addDocument(step))
                            :
                            resultOperations.searchMatchPhraseAlgorithmMono(step.getAlgorithm(), step.getFiveType())
                                    .doOnNext(s -> log.info("search match result algorithm: {}, res: {}", s.getAlgorithm(), s))
                                    .map(matchResult -> FiveStepDocument.next(result, matchResult, step))
                                    .flatMap(nextStep -> stepOperations.searchLastStepMonoByAlgorithm(userId, subnetId, nextStep.getAlgorithm(), fiveType)
                                            .onErrorReturn(nextStep)
                ...
                .doOnNext(s -> log.info("final next step : {}", s))
                .map(FiveStepDto.NextResponse::new);
    }


    public Mono<FiveResultDocument> currentResultDocument() {
        return powerBusProcessor.asFlux()
                .next();
    }
    @Bean
    CommandLineRunner syncFiveResult() {
        return args -> {
            powerBusProcessor.asFlux()
                    .log("sync process run ->>>>>>>>>>>>>>>>")
                    .flatMap(resultOperations::addDocument)
                    .log("sync complete to  elastic")
                    .subscribe();
